...
@section('content')
    ...
    <div class="card shadow mb-4 col-lg-5">
        ...
        <div class="card-body">
            <form action="{{ route('rainfall.update', [$rainfall->station_id, $rainfall->id]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <input type="month" id="monthYear" name="monthYear" value="{{ \Carbon\Carbon::parse($rainfall->date)->format('Y-m') }}" class="form-control" required>
                </div>

                <div class="form-group">
                    <label for="rainfall_amount">Curah Hujan (mm)</label>
                    <input type="number" id="rainfall_amount" name="rainfall_amount" value="{{ $rainfall->rainfall_amount }}" class="form-control" required>
                </div>

                <div class="form-group">
                    <label for="rain_days">Hari Hujan</label>
                    <input type="number" id="rain_days" name="rain_days" value="{{ $rainfall->rain_days }}" class="form-control" required>
                </div>
